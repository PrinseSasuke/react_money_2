...
**Ключевые пары «сбой ↔ атака»** (именно им учится различать модель):
...
- [🔄] M3.5: массовый сбор (рандомизация, `min`-агрегат)
- [ ] M4: RF/MLP/Transformer × 50/75/100%, метрики, матрицы ошибок, важность признаков
- [ ] M5: таблицы/графики и текст для статьи
- [ ] Расширение: криптоджекинг (T1496), эксфильтрация (T1048) — требуют метрик уровня узла и
  сети; подключаются как отдельная волна с расширением `series.py`
